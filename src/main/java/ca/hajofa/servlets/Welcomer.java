 ...
 * @author jonat
 */
public class Welcomer extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String destination = "";
        boolean rememberedUserFound = false;
        Cookie[] cookies = request.getCookies();
        if (cookies != null) {
            for (Cookie c : cookies) {
                if (c.getName().equals("rememberLogin")){
                    if (c.getValue() != null || ! c.getValue().equals("")){
                        if (c.getValue().equals("true")){
                            rememberedUserFound = true;
                        }
                    }
                }
            }
        }
        if (rememberedUserFound){
