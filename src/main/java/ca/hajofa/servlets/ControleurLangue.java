...
public class ControleurLangue extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(langue!=null || request.getParameter("src").equals("1")){
         ...
         /* Recupere le nom de la page d'ou proviens la requete */
        String referer = new URL(request.getHeader("referer")).getPath();
        String[] uriNames = referer.split("/");
        String requestComingFromURL = uriNames[uriNames.length-1];
        System.out.println(new URL(request.getHeader("referer")).getPath());
        String dest = requestComingFromURL;
